...
test("Fill Tractor Enquiry Form with cascading dynamic dropdowns", async ({ page }) => {
  ...
  try {
    ...
    throw err; // rethrow so Playwright still reports the test as failed
  } finally {
    // Ensure language popup is stopped even if test fails
    await languagePopup.stop();
  }
});
